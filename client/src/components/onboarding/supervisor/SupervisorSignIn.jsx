  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await fetch(
        "http://localhost:9000/api/v1/auth/register/supervisor",
        {
          method: "POST",
          headers: {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-1 focus:ring-[#0B4C77]"
        ...
        <div>
          <label className={labelClasses}>Specialization (optional)</label>
          <input
